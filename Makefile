...
all: emulate_kinova_50101.so emulate_kinova_50104.so libemulate_kinova_50104.so libemulate_kinova_50101.so
...
_kinovapy.so: kinovapy.cc emulate_kinova_$(DEFAULT_API_HEADERS_VER).so
	$(CXX) -fPIC -g -DAPI_HEADERS_VER=$(DEFAULT_API_HEADERS_VER) -shared -o $@ -I$(DEFAULT_HEADERS_DIR) -I$(PYTHON_INCLUDE) $< -L.  -lemulate_kinova_$(DEFAULT_API_HEADERS_VER)
